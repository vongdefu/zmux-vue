...
export function serializeTrack(track) {
  ...
  TRACK_KEYS.forEach((key) => {
    if (track[key] !== undefined && track[key] !== null && track[key] !== '') next[key] = track[key];
  });
...
export function loadLibrary() {
  try {
    ...
    return {
      favorites: Array.isArray(data.favorites) ? data.favorites.map(deserializeTrack).filter(Boolean) : [],
      playlists: Array.isArray(data.playlists)
        ? data.playlists.map((playlist, index) => ({
          id: playlist.id || `pl-cached-${index}-${Date.now()}`,
          name: playlist.name || '未命名歌单',
          tracks: Array.isArray(playlist.tracks) ? playlist.tracks.map(deserializeTrack).filter(Boolean) : []
        }))
        : [],
      playHistory: Array.isArray(data.playHistory) ? data.playHistory.map(deserializeTrack).filter(Boolean) : []
    };
  } catch {
...
export function saveLibrary(favorites, playlists, playHistory = []) {
  localStorage.setItem(LIBRARY_STORAGE_KEY, JSON.stringify(createLibrarySnapshot(favorites, playlists, playHistory)));
}
